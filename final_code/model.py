...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    class_filter = [24, 25, 26, 27, 28, 31, 32, 33]
    
    mask_index_filter_list = [ind for ind, ele in enumerate(class_ids) if ele in class_filter]
    
    # mask_index_filter_list = np.ndarray.tolist(np.where(class_ids == 26)[0])
    mask = mask[:, :, mask_index_filter_list]
    class_ids = class_ids[mask_index_filter_list]
    ...
    if use_mini_mask:
        mask, index_flag = utils.minimize_mask(bbox, mask, config.MINI_MASK_SHAPE)
    
    # update mask
    index_flag_list = np.ndarray.tolist(np.where(index_flag == 1)[0])
    # index_flag_list += np.ndarray.tolist(np.where(class_ids == 1)[0])
    # print("index flag: ", index_flag)
    # print("index flag list: ", index_flag_list)
    mask = mask[:, :, index_flag_list]
    # update bbox
    bbox = bbox[index_flag_list]
    # update class_ids
    class_ids = class_ids[index_flag_list]
    
    # print("class_ids ", class_ids)
    # print("mask.shape: ", mask.shape)
    """
    if mask.shape != bmask.shape:
        print("a", mask.shape)
        print("a", bbox.shape)
        print("a", class_ids.shape)
        print("b", bmask.shape)
        print("b", bbbox.shape)
        print("b", bclass_ids.shape)
    """
...
class MaskRCNN():
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        layer_regex = {
            ...
            # "classification": r"(mrcnn_class_logits)|(mrcnn_class)",
            "lastfew":r"(mrcnn_class_logits)|(mrcnn_bbox_fc)|(mrcnn_bbox)|(mrcnn_mask)",
            "classification": r"(mrcnn_class.*)|(mrcnn_bbox.*)",
            "mask": r"(res5.*)|(bn5.*)|(mrcnn\_mask.*)|(rpn\_.*)|(fpn\_.*)",
            "bboxmask": r"(mrcnn_bbox.*)|(res5.*)|(bn5.*)|(mrcnn\_mask.*)|(rpn\_.*)|(fpn\_.*)",
        ...
        train_generator = data_generator(train_dataset, self.config, shuffle=True,
                                         batch_size=self.config.BATCH_SIZE)  # BATCH_SIZE = 2
